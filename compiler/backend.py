...
from compiler.quantize import QuantizationTransform
from compiler.transform import TilingTransform, MappingTransform
from compiler.data import BackendData
...
class Backend:
    ...
    def compile(self, irs: list) -> list:
        # 每个 tiled MatMul → 标准 OS 调度的 MATMUL 指令序列；按层名分组（wi 与 tiling 一致）。
        wi = 0
        for op in irs:
            if isinstance(op, Conv2dIR):
                ...
                wi += 1
                tm, tn, tk = op.tile["tm"], op.tile["tn"], op.tile["tk"]
                num_m = -(-op.M // tm); num_k = -(-op.K // tk); num_n = -(-op.N // tn)
                prog = emit_matmul_program(num_m, num_k, num_n)
                self.data.instr_queue.extend(prog)
                self.data.programs[f"layer{wi}"] = prog
        return irs

    def print_data(self):
        print(f"=== Backend Data ===")
        n = len(self.data.instr_queue)
        print(f"  instr_queue: {n} instrs")
        for i, instr in enumerate(self.data.instr_queue[:6]):
            print(f"    [{i}] {instr}")
        if n > 6:
            print(f"    ... ({n - 6} more)")
