...
def emit_matmul_program(num_m: int, num_k: int, num_n: int) -> list:
    """标准 OS 调度：每个输出 tile (m,n) 累加 num_k 个 K-chunk，按 feed 序（m 外、n 内、k 内）。
    返回 MatMulInstr 列表，accum_addr=tile_id(=m*num_n+n)，is_first=(k==0)、is_last=(k==num_k-1)。
    """
    instrs = []
    tile_id = 0
    for m in range(num_m):
        for n in range(num_n):
            for k in range(num_k):
                instrs.append(MatMulInstr(
                    ub_addr=m * num_k + k,            # A-chunk 线性 tile 索引（功能模型仅作记录）
                    accum_addr=tile_id,
                    length=1,
                    is_first=(k == 0),
                    is_last=(k == num_k - 1),
